#!/usr/bin/env python3
"""Configure Claude Desktop to use Monarch MCP server."""
...
def find_python_executable():
    ...
    if "envs" in python_path and "monarch-mcp" in python_path:
        return python_path
    ...
    try:
        ...
        # Parse output to find monarch-mcp environment
        for line in result.stdout.split('\n'):
            if 'monarch-mcp' in line and not line.startswith('#'):
                parts = line.split()
                if len(parts) >= 2:
    ...
    print("Error: Could not find Python in monarch-mcp conda environment.", file=sys.stderr)
    print("Please activate the monarch-mcp environment and run this script again:", file=sys.stderr)
    print("  conda activate monarch-mcp", file=sys.stderr)
...
def test_server():
    ...
    try:
        ...
        result = subprocess.run(
            [python_path, "-c", "import monarch_mcp.server"],
            capture_output=True,
            text=True,
            cwd=PROJECT_DIR
        )
        if result.returncode != 0:
            print(f"Error: Failed to import monarch_mcp.server", file=sys.stderr)
...
def main():
    ...
    """
    print("--- Monarch MCP Server Configuration for Claude Desktop ---")
